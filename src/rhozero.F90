...
SUBROUTINE RHOZERO
  ...
  IF (EXISTERROR) RETURN; IF (VERBOSE >= 2) WRITE(*,*)"In rhozero.F90 ..."
...
!!$OMP SHARED(NATS,BASIS,ELEMPOINTER,ATOCC,BOZERO,HEF) &
!!$OMP SHARED(HED,HES) &
!!$OMP PRIVATE(I,INDEX,NUMPERORB,NUMPERD,NUMPERS,NUMPERF) &
!!$OMP PRIVATE(NUMPERP)
     DO I = 1, NATS
