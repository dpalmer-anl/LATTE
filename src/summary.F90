...
SUBROUTINE SUMMARY
  ...
  IF (CONTROL .EQ. 2 .AND. SPARSEON .EQ. 1) THEN
     ...
     WRITE(6,'("# Numerical threshold = ",G10.3)') NUMTHRESH
     WRITE(24,'("Numerical threshold = ",G10.3)') NUMTHRESH
  ENDIF
  ...
  IF (ALLOCATED(DIAG_IWORK) .EQV. .TRUE.) THEN
     WRITE(6,'("# Using xSYEVD for diagonalization - potentially fast and potentially unreliable")')
     WRITE(24,'("# Using xSYEVD for diagonalization - potentially fast and potentially unreliable")')
  ELSEIF (ALLOCATED(DIAG_IWORK) .EQV. .FALSE.) THEN
     WRITE(6,'("# Using xSYEV for diagonalization")')
     WRITE(24,'("# Using xSYEV for diagonalization")')
  ENDIF
  ...
  WRITE(6,60)  SYSVOL/REAL(NATS), (TRRHOH + EREP - ECOUL - ENTE + ESPIN)/REAL(NATS), -PRESSURE/TOGPA, &
...
53      FORMAT(I6, 2X, F14.8, 1X, G20.12, 1X,  A1)
54      FORMAT(A2, I6, 2X, F14.8, 1X, G20.12, 1X, A1)
