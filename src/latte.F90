...
PROGRAM LATTE
  ...
  IF( COMMAND_ARGUMENT_COUNT() == 1 ) THEN
     call GET_COMMAND_ARGUMENT( 1, LATTEINNAME )
     WRITE(*,*) "Input file: ", trim(LATTEINNAME)
  END IF

  INQUIRE( FILE=LATTEINNAME, exist=LATTEINEXISTS )
  IF (LATTEINEXISTS) THEN
     IF(.NOT. LIBINIT) CALL PARSE_CONTROL(LATTEINNAME)
  ...
  EPLUSD = ZERO
  IF (PLUSDON .EQ. 1) CALL READPPOTPLUSD 
...
#ifdef PROGRESSON
     IF (DFTBU .AND. COMPFORCE.EQ.1) CALL HUBBARDFORCEPRG
