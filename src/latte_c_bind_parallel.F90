...
SUBROUTINE LATTE_C_BIND (FLAGS, NATS, COORDS, TYPES, NTYPES, MASSES, XLO &
     ...
     VEL, DT, VIRIAL_INOUT, NEWSYSTEM, EXISTERROR,KPOINT_LIST,NKTOT) BIND (C, NAME="latte")
  ...
  INTEGER(C_INT)                 ::  NATS, NTYPES, MAXITER,NKTOT
  ...
  REAL(C_DOUBLE), INTENT(INOUT)  ::  FORCES(3, NATS), VEL(3, NATS), KPOINT_LIST(NKTOT,3)
  ...
  CALL LATTE(NTYPES, TYPES, COORDS, MASSES, XLO, XHI, XY, XZ, YZ, FORCES, &
       MAXITER, VENERG, VEL, DT, VIRIAL_INOUT, NEWSYSTEM, EXISTERROR,KPOINT_LIST,NKTOT)
  PRINT *, "FORCES",FORCES(1,1)
  RETURN
